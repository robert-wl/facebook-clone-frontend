              ...
              <header>
                  ...
                  {/*}*/}
                  {
                        stories?.map((_, index) => {
                            if(index == page)
                            return (
                                <div key={index + Date.now()} className={styles.progress}>
                                    <ProgressBar handlePageChange={handlePageChange} active={null}/>
                                </div>
                            );
                            if (index < page)
                            return (
                                <div key={index + Date.now()} className={styles.progress}>
                                    <ProgressBar handlePageChange={handlePageChange} active={true}/>
                                </div>
                            );
                            if (index > page)
                                return (
                                    <div key={index + Date.now()} className={styles.progress}>
                                       <ProgressBar handlePageChange={handlePageChange} active={false}/>
                                    </div>
                                );
                        })
                  }
